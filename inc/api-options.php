...
function add_api_keys_callback() { ?>
    <div class="wrap"><div id="icon-tools" class="icon32"></div>
        ...
        <form action="<?php echo esc_url( admin_url('admin-post.php') ); ?>" method="POST">
            ...
            <input type="text" name="google_maps_api_key" placeholder="Enter API Key">
...
function submit_api_key() {
    if (isset($_POST['google_maps_api_key'])) {
        ...
        $api_exists = get_option('google_maps_api_key');
        if (!empty($google_maps_api_key) && !empty($api_exists)) {
            update_option('google_maps_api_key', $google_maps_api_key);
        } else {
            add_option('google_maps_api_key', $google_maps_api_key);
        }
    }
